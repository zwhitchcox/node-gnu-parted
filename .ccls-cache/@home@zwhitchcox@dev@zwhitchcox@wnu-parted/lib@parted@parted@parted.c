...
static PedDevice*
_choose_device (char* dev_path, int length)
{
PedDevice*      dev;
  dev = ped_device_get (dev_path);
  dev->length = length;
  if (!dev)
    return NULL;
...
static PedDevice*
_init (char* device_path, int length)
{
PedDevice*      dev;
...
printf("length: %lld\n", length);
dev = _choose_device (device_path, length);
if (!dev)
...
int api(char *dev_path, char *script, long long length) {
  PedDevice*      dev;
  PedDisk*        diskp = 0;
  int             status;
  dev = _init(dev_path, length);
  printf("script: %s\n", script);
  char *scripts[1000];
  // TODO: zwhitchcox
  scripts[0] = script;
  ...
  status = non_interactive_mode(&dev, &diskp, commands, 1, scripts);
